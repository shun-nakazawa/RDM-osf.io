 ...
 * FormFieldInterface has many FormFieldInterface. ArrayFormField or ObjectFormField の場合。
 *
 * QuestionPage: 1ページ分の question のフォームを持つ。suggestion の autofill 処理などを行う。
 * QuestionField: 単一の question のフォーム。FormFieldInterface に加えて、label や help、fill ボタンなどを持つ。
 * FormFieldInterface: 単一の入力フォーム。format によって実装クラスが異なる。
 *
 * ※ ArrayFormField や ObjectFormField が持つのは FormFieldInterface ではなく QuestionField のほうが良いか？
 *    そのほうが、SubQuestion に各種ボタンを装着しやすい。
 *    その場合は、デザインは横ではなく縦型にする必要がある。
 *    横型 or 縦型を format で分けて、横型の場合はボタン付けられない、縦型の場合はボタン付けられる、というふうにするか？
 *
 ******************************************************************************************/
...
const QuestionPage = oop.defclass({
  ...
  // TODO: support hierarchical fields for conditional_required
  // TODO: support hierarchical fields for conditional_enabled
  // validate, enabled のチェック処理自体をここに持ってくるのが良さそう。
  // tree を辿る必要があって、child は parent を知らない状態にしたいので。
  // FormField.validate, QuestionField.validate 自体消す。
  // かわりに QuestionField.setValidationError が必要。
  validateAll() {
    const self = this;
    self.hasValidationError = false;
    self.fields.forEach(function(field) {
      const error = field.validate(self.fields, self.options);
      if (error) {
        self.hasValidationError = true;
      }
      field.showError();
      field.updateEnabled(self.fields);
    });
  },
});
...
const QuestionField = oop.extend(Emitter, {
  ...
  validate: function(questionFields, options) {
    const self = this;
    self.lastError = null;
    try {
      validateField(self.question, self.getValue(), questionFields, options);
    } catch (error) {
  ...
  updateEnabled: function(questionFields) {
    const self = this;
    const cond = self.question.conditional_enabled;
    self.enabled = !cond || evaluateCond(cond, questionFields);
    if (self.enabled) {
      self.element.show();
    } else {
      self.element.hide();
...
function validateRequired(question, value, questionFields, multiple) {
  ...
  const cond = question.conditional_required;
  const condErrorMessage = question.conditional_required_message;
  if (cond) {
    if (typeof(cond) === 'string') {
      const otherField = questionFields.find(function(questionField) {
        return questionField.question.qid === cond;
      });
      if (!otherField) {
        throw new Error('Schema error: invalid conditional_required: ' + cond);
      }
      ...
      if (!evaluateCond(cond, questionFields)) {
        if (!condErrorMessage) {
          throw new Error('Schema error: required conditional_required_message');
        }
