...
def suggestion_metadata(key, keyword, filepath, node):
    logger.info('suggestion key={} keyword={}, filepath={}'.format(key, keyword, filepath))  # TODO: remove debug code
    suggestions = []
...
def _erad_candidates(**pred):
    logger.info('erad candidates: {}'.format(pred))  # TODO: remove debug code
    return [
        dict([
